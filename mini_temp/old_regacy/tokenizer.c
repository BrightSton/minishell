...
//				free_token_list(head); // head 해제 함수
//				return (NULL);
//			}
//		}
//		else
	...
	while (input[i])
	{
		...
		if (input[i] == '|' || input[i] == '>' || input[i] == '<')
		{
			...
			if (input[i] == '>' && input[i + 1] == '>')
				add_token(&head, ">>", TK_REDIR_APPEND), i += 2;
			else if (input[i] == '<' && input[i + 1] == '<')
				add_token(&head, "<<", TK_HEREDOC), i += 2;
			else
			{
				char temp[3] = {input[i], 0, 0};
				add_token(&head, temp, input[i] == '|' ? TK_PIPE :
										input[i] == '>' ? TK_REDIR_OUT :
										TK_REDIR_IN);
				i++;
			}
		}
