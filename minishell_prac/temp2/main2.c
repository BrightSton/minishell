...
///*                                                +#+#+#+#+#+   +#+           */
///*   Created: 2025/04/30 04:50:47 by djang             #+#    #+#             */
///*   Updated: 2025/04/30 04:50:49 by djang            ###   ########.fr       */
///*                                                                            */
///* ************************************************************************** */
...
//// ===== 파이프/리다이렉션 실제 실행 =====
//void execute_cmd_list_with_pipe(t_cmd *cmd_list, t_env *env_list, char **envp) {
//    int prev_pipe_fd[2] = {-1, -1};
//    t_cmd *cmd = cmd_list;

//    while (cmd) {
//        expand_argv_vars(cmd, env_list); // $VAR 확장
...
//        execute_cmd_list_with_pipe(cmd_list, env_list, envp);
...
//        // 🔥 부모에서 직접 실행해야 하는 빌트인 처리
//        if (cmd_list && cmd_list->next == NULL && is_builtin(cmd_list->argv[0]) &&
//            (!strcmp(cmd_list->argv[0], "cd") ||
//             !strcmp(cmd_list->argv[0], "export") ||
//             !strcmp(cmd_list->argv[0], "unset") ||
//             !strcmp(cmd_list->argv[0], "env"))) {
//            expand_argv_vars(cmd_list, env_list); // $VAR 확장
...
//    }
//    close(pipefd[1]);
//    return pipefd[0]; // 읽기용 fd 반환
//}

//// 명령어 실행 전, 히어독 리다이렉션 처리
...
//        // export/unset/env/cd는 반드시 부모에서만 실행
//        if (is_builtin(cmd->argv[0]) &&
...
//    write(1, "\n", 1);
//    exit(130); // bash heredoc Ctrl-C exit code
//}
...
//// ===== 환경변수 등 기존 함수는 동일 (생략) =====
//// ... (생략: init_env, free_env, get_env_value, ms_env, ms_export, ms_unset, env_list_to_envp, free_envp, expand_vars, expand_argv_vars, add_token, parse_line, new_redir, add_redir, is_redir, is_pipe, new_cmd, add_cmd, create_cmd_list, free_cmd_list, is_builtin, exec_builtin, find_path, exec_external) ...
////// ===== 환경변수 =====
//t_env *init_env(char **envp) {
//    t_env *head = NULL, *tail = NULL;
//    for (int i = 0; envp[i]; i++) {
...
//// heredoc을 부모에서 호출, 자식에서 입력받고 fd 반환
//int heredoc_to_pipe(const char *limiter) {
//    int pipefd[2];
//    if (pipe(pipefd) < 0) {
//        perror("pipe");
...
//// ===== 파이프/리다이렉션 실제 실행 =====
//void execute_cmd_list_with_pipe(t_cmd *cmd_list, t_env *env_list) {
//    // heredoc 처리 (Ctrl-C 시 전체 실행 취소)
//    for (t_cmd *cmd = cmd_list; cmd; cmd = cmd->next) {
//        handle_heredoc_redirs(cmd);
//        if (g_heredoc_interrupted) return; // heredoc에서 Ctrl-C 누르면 전체 실행 취소
//    }
...
//        // 🔥 exit 명령어는 언제나 부모에서 바로 처리!
//        if (cmd_list && cmd_list->argc > 0 && !strcmp(cmd_list->argv[0], "exit")) {
//            expand_argv_vars(cmd_list, env_list);
//            exec_builtin(cmd_list, &env_list); // exit(0) 호출됨
//            // exit에서 빠져나오지 않으면 아래로 계속
//        }
//        // 부모에서 직접 실행해야 하는 빌트인
...
int g_exit_code = 0; // 전역 exit code
...
void execute_cmd_list_with_pipe(t_cmd *cmd_list, t_env *env_list) {
    ...
    for (t_cmd *cmd = cmd_list; cmd; cmd = cmd->next) {
        handle_heredoc_redirs(cmd);
        if (g_heredoc_interrupted) return;
    }
